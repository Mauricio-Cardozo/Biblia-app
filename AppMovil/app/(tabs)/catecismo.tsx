...
interface ParteCatecismo  { parte: string; }
interface SeccionCatecismo { seccion: string; }

type NumeralPreview = Pick<CICNumeral, 'id' | 'articulo' | 'texto'>;
...
export default function CatecismoScreen() {
  ...
  const [numerales, setNumerales] = useState<NumeralPreview[]>([]);
  ...
  const cargarSecciones = useCallback(async (parte: string) => {
    setLoading(true); setError(null);
    try {
      // ✅ Query corregida — filtra por parte y trae secciones únicas
  ...
  const cargarNumerales = useCallback(async (parte: string, seccion: string) => {
    setLoading(true); setError(null);
    try {
      const rows = await db.getAllAsync<NumeralPreview>(
  ...
  const cargarDetalle = useCallback(async (id: number) => {
    setLoading(true); setError(null);
    try {
      const row = await db.getFirstAsync<CICNumeral>(
  ...
  const volver = () => {
    if (nivel === "detalle")   { setNivel("numerales"); setDetalle(null); }
  ...
  if (nivel === "detalle" && detalle) {
    return (
      // ✅ View + insets en vez de SafeAreaView → elimina el warning
        ...
        <FlashList
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                {item.articulo ? (
                  <ThemedText style={s.cardSub} numberOfLines={1}>{item.articulo}</ThemedText>
                ) : null}
...
const s = StyleSheet.create({
  ...
  header: {
    ...
    paddingBottom: 14,
    borderBottomWidth: 1, borderBottomColor: C.goldDim,
    gap: 10,
  },
  backBtn: {
    width: 36, height: 36, borderRadius: 18,
    backgroundColor: C.navyLight, borderWidth: 1, borderColor: C.goldDim,
  ...
  card: {
    flexDirection: "row", alignItems: "center",
    backgroundColor: C.navyMid, borderRadius: 10,
    paddingVertical: 12, paddingHorizontal: 12,
    gap: 12, borderWidth: 1, borderColor: C.sep, marginBottom: 6,
  },
